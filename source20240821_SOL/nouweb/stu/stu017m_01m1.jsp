...
public void doQueryEdit(JspWriter out, DBManager dbManager, Hashtable requestMap, HttpSession session) throws Exception
{
	...
	try
	{
		...
		"ROWSTAMP,decode(NATIONCODE,null,'000',NATIONCODE) AS NATIONCODE ,FAX_TEL,PASSPORT_NO,ROWSTAMP, RESIDENCE_DATE ");
...
/** 查多次畢業紀錄 */
public void doQueryUserData(JspWriter out, DBManager dbManager, Hashtable requestMap, HttpSession session) throws Exception {
    Connection conn = null;
    DBResult rs = null;
    try {
        conn = dbManager.getConnection(AUTCONNECT.mapConnect("NOU", session));
		...
		StringBuffer	sql		=	new StringBuffer();
		sql.append("select a.ayear, b.code_name SMS_NAME, a.STNO, ");
		sql.append("to_char(nvl(c.FACULTY_NAME, '')) FACULTY_NAME1, ");
		sql.append("to_char(nvl(d.FACULTY_NAME, '')) FACULTY_NAME2, ");
		sql.append("to_char(nvl(e.FACULTY_NAME, '')) FACULTY_NAME3 ");
		sql.append("from grat003 a ");
		sql.append("join syst001 b on b.kind='SMS' and a.sms=b.code ");
		sql.append("left join syst003 c on c.FACULTY_CODE=a.GRAD_MAJOR_FACULTY ");
		sql.append("left join syst003 d on d.FACULTY_CODE=a.DBMAJOR_GRAD_FACULTY_CODE1 ");
		sql.append("left join syst003 e on e.FACULTY_CODE=a.DBMAJOR_GRAD_FACULTY_CODE2 ");
		sql.append("where stno='"+Utility.dbStr(requestMap.get("STNO"))+"' and grad_reexam_status='2' ");
		sql.append("union ");
		sql.append("select a.ayear, b.code_name SMS_NAME, a.STNO, ");
		sql.append("to_char(nvl(c.FACULTY_NAME, '')) FACULTY_NAME1, '' FACULTY_NAME2, '' FACULTY_NAME3 ");
		sql.append("from grat014 a ");
		sql.append("join syst001 b on b.kind='SMS' and a.sms=b.code ");
		sql.append("join syst003 c on c.FACULTY_CODE=a.FACULTY_CODE ");
		sql.append("where stno='"+Utility.dbStr(requestMap.get("STNO"))+"' and status='2' ");
