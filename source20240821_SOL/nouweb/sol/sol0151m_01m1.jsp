<%/* 
----------------------------------------------------------------------------------
File Name        : sol0151m_01m1.jsp
Author            : barry
Description        : SOL0151M_收件資料登錄審查 - 處理邏輯頁面
Modification Log    :
...
public void doQuery(JspWriter out, DBManager dbManager, Hashtable requestMap, HttpSession session) throws Exception
{
    ...
    try
    {
        ...
        sql.append
        (
            "SELECT SOLT003.NEWNATION,SOLT003.ASYS, SOLT003.AYEAR, SOLT003.SMS, SOLT003.CENTER_CODE, SOLT003.STTYPE, SOLT003.IDNO, SOLT003.BIRTHDATE " +
            "FROM  SOLT003 " +
            "WHERE " +
            "1    =    1 "
        );
...
public void doAdd(JspWriter out, DBManager dbManager, Hashtable requestMap, HttpSession session) throws Exception
{
	...
	try
	{
		...
		if("".equals((String)requestMap.get("REG_FEE")))
			requestMap.put("PAID_AMT", "0");
		else
			requestMap.put("PAID_AMT", (String)requestMap.get("REG_FEE"));
		...
		//舊選轉新全使用原學號
        if(STNO.equals("")){
			//if("0".equals((String) requestMap.get("TOTAL_RESULT")) || "4".equals((String) requestMap.get("TOTAL_RESULT"))){
			if(false){
	        	//取得學號
				...
				stuAdd.setPRE_MAJOR_FACULTY(code.substring(0, 2));
				stuAdd.setJ_FACULTY_CODE(code.substring(2));
				stuAdd.setSEX(requestMap.get("IDNO").toString().substring(1,2));
				...
				stuAdd.setEDUBKGRD((String)requestMap.get("EDUBKGRD"));
		...
		System.out.println("dfasdfasdfasdfsda"+requestMap.get("REG_FEE"));
		if("0".equals((String)requestMap.get("REG_FEE")))
		   fee = "0";
		...
		if ("2".equals(specialStudent) || "4".equals(specialStudent) || "5".equals(specialStudent)) {
			...
			ss.put("STNO", "");
			ss.put("SPECIAL_STTYPE_TYPE", specialStudent);
		...
		if("1".equals((String)requestMap.get("LIMIT")))
			writeoffNo.setPAYMENT_TYPE("1"+Utility.dbStr(requestMap.get("ASYS")));	//繳費類別  1 招生 1空大 2專制 3空研
		else if("2".equals((String)requestMap.get("LIMIT")))
			//writeoffNo.setPAYMENT_TYPE("5"+Utility.dbStr(requestMap.get("ASYS")));	//如果逾期 繳費類別  5 逾期 1空大 2專制 3空研
			writeoffNo.setPAYMENT_TYPE("1"+Utility.dbStr(requestMap.get("ASYS")));	//如果逾期 繳費類別  5 逾期 1空大 2專制 3空研 原報名費列為51工本費，更改為報名費
		...
		writeoffNo.setWR_MANNER("2"); //1.網路2.通信3.現場
		...
		System.out.println("=====PAYMENT_STATUS="+(String) requestMap.get("PAYMENT_STATUS"));
		//已繳費
		if("2".equals((String) requestMap.get("PAYMENT_STATUS"))){
			pi.put("PAID_AMT", (String)requestMap.get("PAID_AMT"));
		}else{//未繳費
			pi.put("PAID_AMT", "0");
		}
		...
		//pi.put("NPAYMENT_BAR", "NON");  //取消沒值時給"NON"的字串
		...
		ei.put("TOTAL_RESULT", (String) requestMap.get("TOTAL_RESULT"));
		// ei.put("AUDIT_UNQUAL_REASON", "NON");
		ei.put("AUDIT_UNQUAL_REASON1", (String) requestMap.get("AUDIT_UNQUAL_REASON1"));
		...
		SOLT004DAO    SOLT004    =    new SOLT004DAO(dbManager, conn, requestMap, session);
        int    updateCount    =    SOLT004.update(condition2);
		...
		if("2".equals((String) requestMap.get("PAYMENT_STATUS"))){
		    ...
		    if("1".equals((String)requestMap.get("LIMIT")))
				...
				SOL1.setPAYMENT_TYPE("5"+requestMap.get("ASYS").toString());    //逾期  5 逾期 1空大 2專制 3空研
		    ...
		    SOL1.setPAID_AMT((String)requestMap.get("PAID_AMT"));                 //實收金額
            ...
            if("1".equals((String)requestMap.get("LIMIT")))
				PCS1.setPAYMENT_TYPE("1" + (String) requestMap.get("ASYS"));		//繳費類別  1 招生 1空大 2專制 3空研
			else if("2".equals((String)requestMap.get("LIMIT")))
				PCS1.setPAYMENT_TYPE("5"+requestMap.get("ASYS").toString());    //如果逾期 繳費類別  5 逾期 1空大 2專制 3空研
			PCS1.setCENTER_CODE((String) requestMap.get("CENTER_CODE"));
            PCS1.setUSER_ID((String)session.getAttribute("USER_ID"));
            PCS1.setPAYMENT_AMT((String)requestMap.get("PAID_AMT"));
			...
			//明細金額
            PCS1.setACNT_AMT((String)requestMap.get("PAID_AMT"));
		...
		if(!Utility.checkNull(requestMap.get("REG_FEE"), "").equals(""))
		{
			PCSINTAMT SOL1  = new PCSINTAMT(dbManager,conn);
		    //並輸入
		    SOL1.setSYS_CODE("SOL");       //系統代號
			...
			if (i == -1) {
	            System.out.println(SOL1.getMessage());
	        } else if (i == 1) {
...
private void doPrint(JspWriter out, DBManager dbManager, Hashtable requestMap, HttpSession session) throws Exception
{
	try
	{
		System.out.println("銷帳編號"+(String) requestMap.get("AYEAR"));
		dbm = dbManager;
        conn = dbManager.getConnection(AUTCONNECT.mapConnect("SOL", session));
...
System.out.println("2");
		...
		System.out.println(getSYST001("SMS",""));
...
System.out.println("3");
        ...
        StringBuffer sql = new StringBuffer().append(" SELECT * FROM ("+
                "SELECT A.NEWNATION,A.AYEAR,A.SMS,A.ASYS,A.BIRTHDATE,A.STNO,A.IDNO "+
        ...
        String C1 = "<img src='/BarcodeServlet?type=Code128B&headless=false&drawText=true&width=1&height=30&fontsize=12&data=";
        String C2 = "'/>";
        ...
        for(int i = 0 ; i<result.size() ; i++){
            ...
            if(Utility.nullToSpace(requestMap.get("ASYS")).equals("1"))
				ht.put("LEFT1","空大報名費");
			else if(Utility.nullToSpace(requestMap.get("ASYS")).equals("2"))
				ht.put("LEFT1","空專報名費");
